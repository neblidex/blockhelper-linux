 ...
 */
 ...
 //- When rebuilding database, change decoded_string to encoded_string
using System;
...
namespace BlockHelper
{
	public partial class Program
	{
		...
		public static void RunBlockHelper(object state)
		{
			...
			if(db_blockheight < rpc_blockheight){
				...
				for(long currentheight = db_blockheight+1;currentheight <= rpc_blockheight;){
					...
					long rpc_batch_size = 25; //Ideal size for writing
				...
				BlockHelperLog("BlockHelper sync completed in " + (UTCTime() - startsync_time) + " seconds");
			}
		...
		public static void LoadDatabase()
		{
			...
			try {
				lockfile = new FileStream(App_Path + "/blockdata/lock.file", FileMode.OpenOrCreate);
				//Because the above doesn't work to lock files alone on Linux, must also use below
				lockfile.Lock(0, 0);
			} catch (Exception) {
				//Ran sync
				lockfile = null;
